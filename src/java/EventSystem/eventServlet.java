...
//        processRequest(request, response);
    ...
    session.setAttribute("dueDate",dueDate);
        try {
            ...
            String query="INSERT INTO `event` (`username`, `type`, `budget`, `contactno`, `address`, `dueDate`) VALUES ('"+session.getAttribute("username")+"', '"+session.getAttribute("type")+"', '"+session.getAttribute("budget")+"', '"+contact+"', '"+address+"', '"+dueDate+"');";
            
            int a=st.executeUpdate(query);
            if(a==1){
                PrintWriter out=response.getWriter();
                out.println("<script type=\"text/javascript\">");
                out.println("alert('Order Successfully inserted!');");
                out.println("</script>");
